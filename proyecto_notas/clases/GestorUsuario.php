...
class Gestorusuario {
    ...
    */
    ...
    /**
     *
     * No se usa, lo escribió en una explicacion de clase.
     * 
     */
...
*/
    ...
    //Método para activar el usuario mediante la comprobación del email
    function activarUsuario($email, $iduser) {
        if (sha1($email . Constantes::SEMILLA) === $iduser) {
           $r = $this->db->updateParameters(self::TABLA, array('estado' => '1'), array('email' => $email));
           return $r;
        } else {
            return -1;
        }    
