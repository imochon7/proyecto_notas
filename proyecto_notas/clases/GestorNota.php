...
class Gestornota{
    ...
    function getUltimoID($id_propietario){
        $this->db->getCursorParametersTop(self::TABLA, '*', array('id_propietario' => $id_propietario));
        if($fila = $this->db->getRow()){
            $objeto = new Nota();
            $objeto->set($fila);
            return $objeto;
    ...
    function getListPaginado($id_propietario, $pc) {
        $this->db->getCursorParameters(self::TABLA);
        return $this->getResultadoSelect();
    }
    
    function getPage($pagina = 1, $id_propietario,  $orderby = 'fecha',  $rpp = 10) {
        $desde = ($pagina - 1) * $rpp;
        $limit = 'limit ' . $desde . ', ' . $rpp;
        $this->db->getCursorParametersMaxNota(self::TABLA, '*', array('id_propietario' => $id_propietario), $orderby, $limit);
        return $this->getResultadoSelect();
    }

    function getResultadoSelect() {
        $respuesta = array();
        while ($fila = $this->db->getRow()) {
            $objeto = new Nota();
            $objeto->set($fila);
