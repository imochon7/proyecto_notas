...
class DataBase {
    ...
    function getCursorTop($tabla, $proyeccion = '*', $condicion = '', array $parametros = array(), $orderby = '1', $limit = '1') {
        if ($condicion === null || trim($condicion) === '') {
            $sql = "select $proyeccion from $tabla order by $orderby $limit";
        } else {
            $sql = "select $proyeccion from $tabla where $condicion order by $orderby DESC LIMIT $limit";
        }
        return $this->send($sql, $parametros);
    }
    
    
    function getCursorMaxNota($tabla, $proyeccion = '*', $condicion = '', array $parametros = array(), $orderby = '1', $limit = '10') {
        if ($condicion === null || trim($condicion) === '') {
            $sql = "select $proyeccion from $tabla order by $orderby $limit";
        } else {
            $sql = "select $proyeccion from $tabla where $condicion order by $orderby DESC $limit";
        }
        return $this->send($sql, $parametros);
    }
    ...
    function getCursorParametersTop($tabla, $proyeccion = '*', array $parametros = array(), $orderby = 'fecha', $limit = '1') {
        $condicion = self::createCondition($parametros);
        return $this->getCursorTop($tabla, $proyeccion, $condicion, $parametros, $orderby, $limit);
    }
    
    function getCursorParametersMaxNota($tabla, $proyeccion = '*', array $parametros = array(), $orderby = 'fecha', $limit = '1') {
        $condicion = self::createCondition($parametros);
        return $this->getCursorMaxNota($tabla, $proyeccion, $condicion, $parametros, $orderby, $limit);
    }
